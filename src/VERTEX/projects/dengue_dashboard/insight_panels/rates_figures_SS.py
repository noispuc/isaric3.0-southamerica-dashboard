...
def create_visuals(
    ...
    for ano in anos:
        ...
        if not df_ano.empty:
            dfs_casos.append(df_ano[["ano", "Without warning signs"]])
    if dfs_casos:
        df_casos_all = pd.concat(dfs_casos, ignore_index=True)
        df_casos_all.rename(
            columns={"Without warning signs": "Confirmed cases rate"},
            inplace=True,
        )
    ...
    if dfs_hosp:
        ...
        fig_hosp, gid_hosp, glab_hosp, gabout_hosp = idw.fig_bar_chart(
            ...
            graph_about=(
                "Proportion of confirmed dengue cases (classi_fin 10) "
    ...
    if dfs_letal:
        ...
        fig_letal, gid_letal, glab_letal, gabout_letal = idw.fig_bar_chart(
            ...
            graph_about=(
                ...
                "divided by confirmed cases (classi_fin IN 10), by year."
            ),
        )
