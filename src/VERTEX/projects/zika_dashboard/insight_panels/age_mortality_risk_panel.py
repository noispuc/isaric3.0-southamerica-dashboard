...
def _build_mortality_figure(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title="Age-adjusted mortality risk",
        xaxis_title="Age (years)",
        yaxis_title="Adjusted mortality risk",
...
def create_visuals(
    ...
    if not df_curve.empty:
        ...
        graph_label = "Age-adjusted mortality risk"
        graph_about = (
            "Dengue mortality risk curve by age, "
            "calculated as the proportion of deaths among confirmed cases (classi_fin 10, 11, 12), "
            "consolidating all available years and smoothed using a moving average. "
            "The gray area represents an approximate confidence interval (95%)."
        )
