...
def _load_sinan_view(year: int = 2024) -> pd.DataFrame:
    """
    Lê diretamente a VIEW sinan_zika.vw_casos_tab12_base.
    ...
    sql = text(
        """
        SELECT *
        FROM sinan_zika.vw_casos_tab12_base_new
        WHERE ano = :ano
        """
    )
    try:
        with engine.connect() as conn:
            print("[TABLE2] Lendo sinan_zika.vw_casos_tab12_base…", flush=True)
