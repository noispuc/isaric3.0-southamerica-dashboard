...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    df_map["outco_lengthofstay"] = (df_map["outco_date"] - df_map["pres_date"]).dt.days
    ...
    df_model = df_model.loc[(df_model["outco_binary_outcome"].isin(["Death", "Discharged"]))]
    df_model["outco_binary_outcome"] = (df_model["outco_binary_outcome"] == "Death").astype(int)
    ...
    df_model["vital_gcs___Moderate"] = (df_model["vital_gcs"].isin([9, 10, 11])).astype(object)
    ...
    df_model["labs_platelets_103ul___Low"] = df_model["labs_platelets_103ul"] < 1.5
    df_model["labs_platelets_103ul___High"] = df_model["labs_platelets_103ul"] > 4.5

    dictionary_columns = ["field_name", "form_name", "field_type", "field_label", "parent"]
    dictionary_values = [
        ["comor_chrkidney_stag___Stage 3b or 4 or 5", "presentation", "binary", "Stage 3b or 4 or 5", "comor_chrkidney_stag"],
        ["vital_gcs___Moderate", "daily", "binary", "Moderate (9 to 11)", "vital_gcs"],
        ["vital_gcs___Severe", "daily", "binary", "Severe (less than 9)", "vital_gcs"],
        ["labs_platelets_103ul___Low", "daily", "binary", "Low (under 1.5)", "labs_platelets_103ul"],
        ["labs_platelets_103ul___High", "daily", "binary", "High (over 4.5)", "labs_platelets_103ul"],
    ]
    predictors_dict = pd.DataFrame(dictionary_values, columns=dictionary_columns).to_dict(orient="list")
    ...
    valid_predictors_vif, iterative_vif = ia.variance_influence_factor_backwards_elimination(
        df_model, dictionary, predictors_list=predictors
    )

    valid_predictors_outcome = ia.remove_single_binary_outcome_predictors(
        df_model, dictionary, predictors_list=predictors, outcome_str="outco_binary_outcome"
    )

    rename_columns = dict(zip(predictors, ["var" + str(n) for n in range(len(predictors))]))
    ...
    logr_results = pd.merge(multivariate_logr_results, univariate_logr_results, on="Study", how="outer")
    # -----

    # ----- Is it possible to move this into execute_glm_regression
    logr_results["p-value (multi)"] = logr_results["p-value (multi)"].astype(float)
    logr_results["p-value (uni)"] = logr_results["p-value (uni)"].astype(float)
    logr_results = logr_results.rename(columns={"Study": "Variable"})

    logr_results["Variable"] = logr_results["Variable"].apply(lambda x: "___".join(x.strip("]").split("[")).split("___True")[0])

    logr_results["Variable"] = logr_results["Variable"].apply(lambda x: x.split("___")[0]).map(
        dict(zip(["var" + str(n) for n in range(len(predictors))], predictors))
    ) + logr_results["Variable"].apply(lambda x: "___" + x.split("___")[-1] if "___" in x else "")
    # -----

    highlight_predictors = {
        "+": [var for var in logr_results["Variable"].values if var not in valid_predictors_vif],
        "†": [var for var in logr_results["Variable"].values if var not in valid_predictors_outcome],
    }
    pvalue_significance = {"*": 0.05, "**": 0.01}
    logr_results_table = ia.regression_summary_table(
        logr_results.copy(), dictionary, highlight_predictors=highlight_predictors, pvalue_significance=pvalue_significance
    )
...
analysis""",
    ...
    table_key = "<br>".join([f"({k}) p < {str(v)}" for k, v in pvalue_significance.items()]) + "<br>".join(
    ...
    labels = ["Variable", "OddsRatio (multi)", "LowerCI (multi)", "UpperCI (multi)", "p-value (multi)"]
