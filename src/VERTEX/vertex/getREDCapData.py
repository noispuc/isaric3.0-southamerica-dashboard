...
def user_assigned_to_dag(redcap_url, redcap_api_key):
    conex = {"token": redcap_api_key, "content": "dag", "format": "csv", "returnFormat": "json"}
...
switch DAG to unique group name: {unique_group}")
            ...
            try:
                ...
                logger.warning(
                    ...
                    f"Warning: Could not retrieve data from unique group name: {unique_group}"
                )
                continue
        if len(df_list) > 0:
...
def get_data_dictionary(redcap_url, redcap_api_key):
    ...
    conex = {"token": redcap_api_key, "content": "metadata", "format": "csv", "returnFormat": "json"}
...
def get_form_event(redcap_url, redcap_api_key):
    ...
    conex = {"token": redcap_api_key, "content": "event", "format": "csv", "returnFormat": "json"}
    # Make the API request
    response = requests.post(redcap_url, data=conex)
    ...
    conex = {"token": redcap_api_key, "content": "instrument", "format": "csv", "returnFormat": "json"}
    # Make the API request
    response = requests.post(redcap_url, data=conex)
    if response.status_code == 200:
        form = pd.read_csv(io.StringIO(response.text), keep_default_na=False)
        form = form.rename(columns={"instrument_name": "form", "instrument_label": "form_label"})
    else:
        form_columns = ["form", "form_label"]
        form = pd.DataFrame(columns=form_columns)

    conex = {"token": redcap_api_key, "content": "formEventMapping", "format": "csv", "returnFormat": "json"}
    # Make the API request
    response = requests.post(redcap_url, data=conex)
    if response.status_code == 200:
        form_event = pd.read_csv(io.StringIO(response.text), keep_default_na=False)
    else:
    ...
    form_event = form_event.groupby(["arm_num", "unique_event_name"]).agg(lambda x: ",".join(x)).reset_index()
    form_event = pd.merge(form_event, event, on=["unique_event_name", "arm_num"], how="left")
...
def get_missing_data_codes(redcap_url, redcap_api_key):
    ...
    conex = {"token": redcap_api_key, "content": "project", "format": "csv", "returnFormat": "json"}
    response = requests.post(redcap_url, data=conex)
    df = pd.read_csv(io.StringIO(response.text), keep_default_na=False)
...
def add_onehot_variables(data, dictionary, sep="___"):
    ...
    new_dictionary.loc[ind, "parent"] = new_dictionary.loc[ind, "field_name"].apply(lambda x: x.split("_")[0])
    ...
    new_variables.loc[:, "field_label"] = new_variables["field_name"].apply(lambda x: x.split("___")[-1])
    new_variables.loc[:, "parent"] = new_variables["field_name"].apply(lambda x: x.split("___")[0])
    ...
    sections["field_label"] = new_dictionary.loc[ind, "section_header"].apply(lambda x: x.split(": ")[0])
    sections["field_type"] = "section"
    sections["form_name"] = new_dictionary.loc[ind, "form_name"]
    sections["field_name"] = new_dictionary.loc[ind, "field_name"].apply(get_section_prefix)
    sections.index -= 0.5
...
def convert_dictionary_field_type(dictionary):
    ...
    numeric_ind = new_dictionary[val_column].isin(["number", "integer"]) | new_dictionary["field_type"].isin(["slider"])
...
def replace_with_nan_for_missing_code_checkbox(df, missing_data_codes):
    ...
    missing_columns = [col for col in df.columns if col.split("___")[-1] in missing_data_values]
...
def combine_unlisted_variables(df, dictionary, sep="___"):
    ...
    unlisted_columns_dict = {k: [v for v in unlisted_item_columns if k in v] for k in unlisted_columns}
    ...
    for ind in unlisted_columns.index:
        ...
        df = pd.concat([df.iloc[:, :column_loc], new_df, df.iloc[:, column_loc:]], axis=1)
        new_dictionary_index = ind + np.linspace(0.1, 0.9, len(values))
        new_dictionary = pd.DataFrame("", columns=dictionary.columns, index=new_dictionary_index)
...
def resolve_checkbox_branching_logic(df, dictionary):
    ...
    branching_logic_variables = dictionary["branching_logic"].apply(get_branching_logic_variables)
    for ind in checkbox_ind.index:
        branching_logic_columns = [col for col in df.columns if col in branching_logic_variables.loc[ind]]
        remove_ind = df[branching_logic_columns].isna().any(axis=1)
        checkbox_columns = [col for col in df.columns if (col.split("___")[0] == dictionary.loc[ind, "field_name"])]
...
def homogenise_variables(df, dictionary):
    ...
    for index, row in conversion_table.iterrows():
        ...
        try:
            ...
            check_ind = value_col in ["labs_lymphocyte", "labs_neutrophil"] and from_unit == "10^9/L" and to_unit == "%"
            if check_ind:
                ...
                if total_wbc_col in df.columns:
                    ...
                    df.loc[:, total_wbc_col] = pd.to_numeric(df[total_wbc_col], errors="coerce")
                    # Apply conversion only to non-empty values
                    mask = (df[unit_col] == from_unit) & df[value_col].notna() & df[total_wbc_col].notna()
                    df.loc[mask, value_col] = 100 * (df.loc[mask, value_col] / df.loc[mask, total_wbc_col])
...
def initial_data_processing(data, dictionary, missing_data_codes):
    ...
    remove_variables = [
        x for x in remove_columns.map(lambda x: x.split("___")[0]) if x not in data.columns.map(lambda x: x.split("___")[0])
    ]
    new_dictionary["section_header"] = new_dictionary["section_header"].replace("", np.nan)
    new_dictionary["section_header"] = new_dictionary["section_header"].ffill()
    new_dictionary = new_dictionary.loc[(new_dictionary["field_name"].isin(remove_variables) == 0)]
