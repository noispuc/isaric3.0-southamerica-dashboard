...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    for ano in anos:
        ...
        if not df_ano.empty:
            dfs_casos.append(df_ano[["ano", "Sem sinais de alarme"]])

    if dfs_casos:
        df_casos_all = pd.concat(dfs_casos, ignore_index=True)
        df_casos_all.rename(
            columns={"Sem sinais de alarme": "Taxa de casos confirmados"},
            inplace=True,
        )
    ...
    if dfs_hosp:
        ...
        fig_hosp, gid_hosp, glab_hosp, gabout_hosp = idw.fig_bar_chart(
            ...
            graph_about=(
                "Proporção de casos de dengue confirmados (classi_fin 10) "
    ...
    if dfs_letal:
        ...
        fig_letal, gid_letal, glab_letal, gabout_letal = idw.fig_bar_chart(
            ...
            graph_about=(
                ...
                "divididos pelos casos confirmados (classi_fin IN 10), por ano."
            ),
        )
